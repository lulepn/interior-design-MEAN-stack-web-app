...
@Component({
  selector: 'app-pregled-posla-klijent',
  templateUrl: './pregled-posla-klijent.component.html',
  styleUrls: ['./pregled-posla-klijent.component.css']
})
export class PregledPoslaKlijentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.servisO.dovuciObjekatAdresa(this.adresa).subscribe((data: Objekat)=>{
      this.objekat = data ;
      this.servisU.searchAgencija(sessionStorage.getItem('agencija')).subscribe((data2 : Agencija)=>{
        this.agencija = data2;
  ...
  otkazi() {
    let otkazivanje = new Otkazivanje;
    otkazivanje.adresaObj = this.posao.adresaObj;
    otkazivanje.agencija = this.posao.agencija;
    otkazivanje.usernameObj = this.posao.usernameObj;
    this.servisP.dodajOtkazivanje(otkazivanje).subscribe((resp)=>{
